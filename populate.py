...
def load_elo_data(rows=slice(0,None)):
    df = pd.read_excel('data-and-ratings.xlsx', sheet_name='EEG_RatingV2_Score')
    try:
        df = df[rows]
    except KeyError:
        df = df.loc[rows]
    ...
    mapping = pd.read_excel('data-and-ratings.xlsx', sheet_name='Item_mappings').set_index('Item_no')
