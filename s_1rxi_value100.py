...
definitions = {
    **common.definitions,
    'value': 'Value: Estimate the potential commercial value of the idea on a scale of 1 to 100, while disregarding its novelty and feasibility.'
}
...
async def rate(data):
    ...
    for qid in qids:
        ...
        samples = {measure: get_samples_string(measure, qid) for measure in definitions.keys()}
        ...
        for ichunk, chunk in enumerate(chunks):
            ...
            for measure, definition in definitions.items():
                prompt = []
                for p in prompts:
...
def get_samples_string(measure, qid):
    items = annotate.get_samples(measure, qid).sort_values(f'{measure}_transformed').reset_index(drop=True, inplace=False)
